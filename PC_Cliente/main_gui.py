...
class JuegoStranger:
    def __init__(self, root):
        ...
        try:
            ...
            print("DEBUG DE IMAGEN")
            print("Ruta exacta que Python está buscando:", ruta_imagen)
            print("El archivo realmente existe ahí:", os.path.exists(ruta_imagen))
            
            if not os.path.exists(ruta_imagen):
                print("ALERTA: Python confirma que el archivo NO está en esa ruta exacta.")
                self.img_dicc = None
            else:
                self.img_dicc = tk.PhotoImage(file=ruta_imagen)
                
        except Exception as e:
            print("ERROR: El archivo existe, pero Tkinter no puede leerlo:", e)
            self.img_dicc = None
    ...
    def conectar_wifi(self):
        try:
            ...
            self.lbl_estado_wifi.config(text="WiFi Falló. Está la Pico prendida?", fg="red")
    ...
    def lanzar_frase_simple(self):
        ...
        try:
            self.socket_cliente.send(f"FRASE:{self.frase_actual}:{self.nivel_actual}".encode())
        except:
    ...
    def nueva_frase_escucha(self):
        ...
        self.lbl_turno.config(text="TURNO A (PC) - Atento a la maqueta")
        self.btn_validar_A.config(state='normal')
        self.btn_validar_B.config(state='disabled')
        self.log_consola(f"\nRONDA {self.ronda} INICIADA")
        
        try: 
            self.socket_cliente.send(f"FRASE:{self.frase_actual}:{self.ronda}".encode())
        except: 
            self.log_consola("Error enviando datos.")
    ...
    def validar_A_escucha(self):
        ...
        self.lbl_turno.config(text="TURNO B (MAQUETA) - Usa el botón")
